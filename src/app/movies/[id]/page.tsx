...
const MovieDetails = () => {
 ...
 useEffect(() => {
  const fetchReactions = async () => {
    ...
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/movies/${id}/reactions/`, {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      ...
      try {
        const response = await axios.get<Reviews[]>(`http://127.0.0.1:8000/api/ratings/${id}/`, {
          headers: {
  ...
  useEffect(() => {
    ...
    const fetchMovieDetails = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/movie/api/movies/${id}/`);
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/movies/${id}/comments/`);
      ...
      <div className="z-10 relative max-w-4xl mx-auto p-6 text-white shadow-xl rounded-lg border border-gray-800">
        ...
        <div className="flex items-center mt-6 space-x-6">
          <div className="flex items-center">
            ...
            <ReactStars count={5} value={rating} size={24} edit={true} color2="#ffcc00" onChange={handleRating} />
            <span className="ml-2 text-gray-300 font-semibold">{rating ? Number(rating).toFixed(1) : "0.0"}</span>
          </div>
          <button
            ...
            <div className="max-h-60 overflow-y-auto mt-2 space-y-3">
              {comments.length > 0 ? (
                comments.map((comment) => (
                  <div key={comment.id} className="p-3 border-b border-gray-700 bg-gray-800 rounded-md">
