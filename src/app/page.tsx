      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 px-6 pl-14 pt-6">
        {filteredMovies.length > 0 ? (
          filteredMovies.map((movie) => (
            <div key={movie.id} className="bg-[#1A1D24] p-3 rounded-xl shadow-md transition-transform transform hover:scale-105">
              <div className="relative w-full h-[180px]">
                <Image src={movie.image} alt={movie.title} layout="fill" objectFit="cover" className="rounded-md" />
              </div>
              ...
              {/* Ratings */}
              <div className="mt-2">
                <ReactStars count={5} value={movie.total_ratings} size={16} edit={false} color2={"#FFD700"} />
